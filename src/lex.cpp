 ...
 *  Numeric { "numeric", { num, ... } }
 *  Numeric Range: { "numeric_range", { lower, upper } }
...
int_pair_t Variable::lex(std::string& msg) {
    ...
    char c;                           // Character currently studying
    ...
    for (pos = 0; pos < lim;) {
        ...
        if (token.type == "") {
            if (c == ' ')
                ...
                // ALTERNATIVE
                if (tokens.size() == 0) {
                    msg = "error: premature alternative";
                    return {pos, 1};
                } else {
                ...
                // Cannot have multiple repetitions in a row
                int tsize = tokens.size();
                if (tsize > 0 && tokens[tsize - 1].type == "repetition") {
                    msg =
                        "error: expected group or terminal to follow "
                        "repetition";
                    return {pos, 1};
                } else {
            ...
            if (token.spos == 0) {
                token.spos = pos;  // Set starting position
            }
        } else if (token.type == "string") {
            if (token.spos == 0) token.spos = pos;
            ...
            if (is_digit(c)) {
                *get_last(token.data) += c;
                ...
                if (token.data.size() == 2 && token.data[1] == "")
                    token.data[1] = std::to_string(LONG_MAX);
                // Single-repitition?
                if (token.data.size() == 1) {
                    token.data.push_back(
                        std::to_string(std::stol(token.data[0]) + 1));
                }
                tokens.push_back(token);
                token = {};
                inc = 0;
    ...
    for (Token& token : tokens) {
        ...
        if (token.type == "numeric" || token.type == "numeric_range") {
            ...
            if (token.data[0][0] != 'd') {
                ...
                for (int j = 1; j < nums; j++) {
                    token.data[j] =
                        std::to_string(std::stol(token.data[j], nullptr, base));
                }
            }
            // Remove base
        ...
        // Check range
        if (token.type == "numeric_range" || token.type == "repetition") {
            int lower = std::stol(token.data[0]);
            if (token.data.size() == 1) {
                token.data.push_back(std::to_string(lower + 1));
            }
            int upper = std::stol(token.data[1]);
            if (upper <= lower) {
                msg = "error: negative range " + token.data[0] + "-" +
                      token.data[1];
                return {token.spos, token.length - 1};
            }
        }
    }
